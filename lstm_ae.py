...
class TrainSequiturLSTMAE():
    ...
    def __init__(self,encoding_dim=32, h_dims=[128], h_activ=nn.Sigmoid(), out_activ=nn.Tanh(), config: Config = Config()) -> None:
        ...
        self.net = net = LSTM_AE(encoding_dim=encoding_dim, input_dim= config.n_channels, h_dims=h_dims, h_activ=h_activ,out_activ=out_activ, config=config, use_hidden=False)


        self.X_train, self.y_train = load_data('train',config.len_seq,config.stride, removeAnom=True)
